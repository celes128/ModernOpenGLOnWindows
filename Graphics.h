...
public:
	...
	miny::Result	Initialize(in_ HWND hwnd, in_ SIZE winSize, in_ HINSTANCE hinstance);
	void			Terminate();

	void			Present();

private:
	miny::Result	LoadOpenGL(HINSTANCE hinstance);
	miny::Result	LoadOpenGLFunctions(HWND hwnd);
	miny::Result	LoadExtensionList();
	miny::Result	InitializeOpenGL(HWND hwnd, SIZE winSize);
